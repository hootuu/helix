...
var gFactoryCodeRegexpTpl = `^[0-9][A-Za-z0-9_]{0,32}*$`
...
func CheckFactoryCode(code string) error {
	...
	if !matched {
		return errors.New("invalid factory code[" + gFactoryCodeRegexpTpl + "]: " + code)
	}
	return nil
}

var gIdemCodeRegexpTpl = `^[0-9][A-Za-z0-9_.-]{0,127}$`
